...
if pagina == 'Analyse fietstochten':
    ...
    # 1. Set max slider value based on current filtered data
    max_slider = int(filtered_fiets_plot['weekly_total_users'].max())

    # 2. Initialize the slider value in session_state only once
    if slider_key not in st.session_state:
        st.session_state[slider_key] = 0  # or any sensible default

    # 3. Render the slider using only key — this avoids the warning
    MAX_SLIDER_LIMIT = 6000  # Choose a high enough ceiling

    min_fietsritten = st.slider(
        "Minimaal aantal fietsritten",
        min_value=0,
        max_value=MAX_SLIDER_LIMIT,
        key="min_fietsritten_slider"
    )

    filtered_fiets_plot = filtered_fiets_plot[
    filtered_fiets_plot['weekly_total_users'] >= st.session_state["min_fietsritten_slider"]
]

    # 4. Use the value from session state
    filtered_fiets_plot = filtered_fiets_plot[filtered_fiets_plot['weekly_total_users'] >= st.session_state[slider_key]]
